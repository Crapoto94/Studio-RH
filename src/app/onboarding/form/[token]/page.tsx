...
import { Loader2, CheckCircle, Send, AlertCircle, FileText, ChevronRight, ChevronLeft, Search, User, HelpCircle } from 'lucide-react'
...
export default function OnboardingFormPage({ params }: OnboardingFormPageProps) {
  ...
  if (submitted || data.onboarding.statut === 'en_cours_realisation' || data.onboarding.statut === 'termine') {
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 p-4">
        <Card className="max-w-md w-full text-center p-8 rounded-3xl border-none shadow-xl bg-white">
          <div className="w-20 h-20 bg-emerald-100 text-emerald-600 rounded-full flex items-center justify-center mx-auto mb-6">
            <CheckCircle size={40} />
          </div>
          <CardTitle className="text-2xl font-display font-bold text-slate-800">C'est envoyé !</CardTitle>
          <CardDescription className="text-slate-500 mt-2">
            La DSI a bien reçu vos besoins. Nous préparons maintenant l'arrivée de l'agent.
          </CardDescription>
        </Card>
      </div>
    )
        ...
        <div className="space-y-6">
          ...
          {currentStep.isSummary ? (
            <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden divide-y divide-slate-100">
              {steps.filter(s => !s.isSummary && s.groups).map(step => (
                          ...
                          {field.type === 'multiselect' && (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-1">
                              {lists[field.options]?.map((opt: any) => {
                                ...
                                return (
                                  <label key={name} className={`flex items-center gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all ${isChecked ? 'bg-indigo-50 border-indigo-100 font-bold' : 'bg-white border-slate-50 hover:border-slate-100'}`}>
                                    <input
                                      type="checkbox"
